...
- 解读采用渐进披露：每日一牌与单牌/三牌的默认生活指引固定提供财运、事业、爱情三个页签且一次只显示一段；单牌/三牌可切换为具体问题，由模型直接识别重点，不要求预选感情/人际/事业/自我类别。首屏将可选用户困扰与“第一眼线索”分区展示，三牌一次只读一张，牌义依据默认折叠；旧主题字段和 v0.2 多维字段仅用于本地历史兼容。
- v1.0 主题解读使用维护者自有 CloudBase 环境的内置 AI Provider `cloudbase` 与模型 `hy3`；环境 ID 只保存在被 Git 忽略的本地配置中，调用入口为小程序端 `wx.cloud.extend.AI.createModel`，不得把模型密钥或 AppSecret 写入仓库。
- 真实 AI 只接收 `focusMode`、可选当前问题、牌阵、卡牌事实与受控牌义，不读取本地历史；生活指引不得伪造用户问题，固定方向顺序为财运、事业、爱情。`summary` 不得复述或套话改写用户问题，输出必须通过结构、牌面事实、模式焦点和安全校验，失败统一回退到本地受控牌义。
