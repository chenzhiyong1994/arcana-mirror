...
当前实现由 `apps/miniprogram/components/tarot-card-face/` 负责成牌呈现：22 张大阿尔卡那保留原图内已经烘焙的共享框、0—XXI 编号和铭牌，56 张无框小阿尔卡那通过组件叠加 `shared/front-frame-overlay.png` 的运行时缩放图，以及顶部花色内牌阶标记和底部中英文铭牌。小牌的 A、II—X、PAGE、KNIGHT、QUEEN、KING 是牌阶，不是整副牌的 22—77 全局序号。两者使用同一框体原件，不再以 CSS 近似绘制。正逆位旋转完整成牌，所有页面不得绕过组件直接展示已解锁牌面。
