...
**包含：**22 张大阿尔卡那正式卡面、正逆位、单牌、三牌、每日一牌、CloudBase AI 聚焦主题解读、本地降级、可选本地历史、图鉴收集、安全分类，以及支持定位故障的最小日志。
...
| 字段 | 类型 | 规则 |
| --- | --- | --- |
| summary | string | 不超过 260 字，概括当前值得关注的张力 |
| cards | array | 数量与抽牌数一致；逐张包含牌名、牌位、依据、情境解释、主题名和主题聚焦解读 |
...
结果页按“总览 → 牌面 → 当前牌的情境解释与主题解读 → 可展开牌义依据 → 综合提示 → 反思问题与微行动 → 保存/结束”渐进展示。三牌默认只展开一张，用户点牌切换；来源标识与免责声明降为辅助信息，不抢占首屏。
