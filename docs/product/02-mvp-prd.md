...
- **地点**：相对私密的移动端环境；
- **起因**：用户反复思考但无法形成新角度；
- **经过**：选择牌阵并输入问题；系统识别高风险内容并帮助改写问题；完成抽牌并等待 AI 解读；
...
- **当前技术方案**：微信小程序本地领域服务负责抽牌、安全策略、本地历史和降级；主题解读通过 CloudBase AI Provider 直接调用 `hy3`，输出仍经相同契约校验。
- **验证方案**：功能测试、AI 离线评估集和小规模体验测试共同验证。

### 5.3 MVP 范围

**包含：**78 张完整牌组正式卡面、正逆位、单牌、三牌、每日一牌、CloudBase AI 问题聚焦解读、本地降级、可选本地历史、分组图鉴、安全分类，以及支持定位故障的最小日志。
...
- **流程影响**：新建产品，无旧流程迁移；
- **数据影响**：每日牌与已完成主题解读自动进入当前设备最近 30 条历史，均视为私密内容；用户可单条删除；
- **外部影响**：主题问题会发送至项目 CloudBase 环境的 AI 服务；不读取账号资料或历史记录；
...
| 字段 | 类型 | 必填 | 规则 |
| --- | --- | --- | --- |
| 问题焦点 | 自动识别 | 是 | 不要求用户预选固定类别；模型根据问题识别重点，仍受专业与安全边界约束 |
...
真实 AI 仅用于主题问题解读；每日一牌使用静态牌义和预置反思问题。小程序通过 CloudBase 内置 Provider `cloudbase` 调用 `hy3`。输入只包含当前问题、主题、牌阵、卡牌、牌位、朝向和受控牌义，不包含任何账号资料或历史记录。问题应视为会发送给外部 AI 处理的数据，界面提示用户不要输入姓名、电话、住址等可识别信息；CloudBase 控制台可能保留调用记录。

AI 必须返回以下结构。v0.3 起逐牌只返回用户已选主题的一条聚焦解读，不再同时铺开爱情、财运、事业与自我成长四个维度：

| 字段 | 类型 | 规则 |
| --- | --- | --- |
| summary | string | 不超过 260 字，直接给出新的核心观察；不得复述、引用或套话改写用户问题 |
| cards | array | 数量与抽牌数一致；逐张包含牌名、牌位、依据、情境解释、问题焦点标签和聚焦解读 |
...
结果页按“总览 → 独立框选的原始困扰 → 第一眼线索 → 牌阵与当前牌解读一体区 → 可展开牌义依据 → 综合提示 → 反思问题与微行动 → 可选分享”渐进展示。三牌默认只展开一张，真实卡面直接作为当前解读的就近切换入口；免责声明降为辅助信息，不抢占首屏，页面不再展示 AI 来源标识。结果页不设置额外“完成”按钮，退出统一使用系统左上角返回或侧滑返回。
...
不设置独立设置页。AI 与敏感信息边界在提问页就近提示，娱乐性声明在结果页展示；历史页直接说明最近 30 条与仅存当前设备的边界。
