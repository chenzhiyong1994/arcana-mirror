...
| 2026-07-21 | 纳入三牌、丰富解读、图鉴与正式卡面 | 进入 v0.2 本地体验扩展 | Codex | 项目作者确认 | v0.4 |
| 2026-07-25 | 直接扩展为 78 张完整牌组，不新增回归型使用循环 | 进入 v1.1 完整牌组候选 | Codex | 项目作者确认 | v1.1 |
| 2026-07-23 | 接入 CloudBase AI `hy3` 并准备正式版 | 主题解读使用真实模型，本地历史与安全降级保持不变 | Codex | 项目作者确认 | v1.0 |

> v0.2 增量需求、数据边界和验收标准见 [`08-v0.2-three-card-and-collection.md`](08-v0.2-three-card-and-collection.md)。本文件中“仅单牌、三牌后置”的描述仅代表 v0.1 基线，已被该增量 PRD 覆盖。
...
3. **个人项目容易失控**：78 张牌已通过生成合同、分包和自动化门禁收口；更多牌阵、社区、付费、真人服务仍会显著增加开发和内容成本。
4. **展示价值容易停留在界面层**：如果缺少数据、降级、测试和部署方案，难以体现完整产品和工程能力。

### 1.3 解决思路

围绕“提出困扰—选择单牌或三牌—抽牌—情境化解读—记录行动—点亮图鉴”构建本地闭环；将 AI 限定为解释和反思辅助者，用结构化输出、安全策略、规则校验和本地降级保障体验。v1.1 覆盖完整 78 张、单牌及三牌解读；不建设签到、回访或连续使用循环，更多惊喜来自一次抽牌即可遇到的牌面与组合。
...
- **起因**：想快速获得当天值得关注的提示；
- **经过**：进入首页，点击每日一牌，完成短洗牌和翻牌；结果按财运、事业、爱情三个固定方向提供本地提示，使用页签一次只展示一段，再收束到反思问题；
- **结果**：可保存一句感受，结果进入日历，同一天默认不重复抽取。

### 2.2 核心场景二：单牌或三牌解读

- **人物**：想从牌面整理生活状态的成年用户，不要求其已经形成明确问题；
- **时间**：困扰持续出现、需要整理思路时；
- **地点**：相对私密的移动端环境；
- **起因**：用户想获得新角度，或正反复思考某个具体困扰；
- **经过**：默认选择“生活指引”，无需输入即可按财运、事业、爱情三个固定方向解读；也可切换为“具体问题”，输入后先完成安全识别与必要改写，再进入单牌或三牌仪式；
- **结果**：获得牌义依据、情境解读、反思问题和一个微行动，自动进入当前设备最近 30 条记录，可随时删除。
...
| 3 | 项目范围过大导致无法完成 | 项目整体 | 高 | 78 张牌通过合同化生成和资产门禁收口；仍限两种牌阵、无商业模块 |
...
3. 每日牌与主题解读完成后自动进入本地历史，并只保留最近 30 条；
4. 高风险输入不会进入普通生成链路；
5. CloudBase AI 超时、返回非法结构或内容校验失败时能够降级；
6. 本地历史只在当前设备的小程序存储沙箱内可见，并支持单条删除；
7. 小程序通过 CloudBase 内置 AI 能力调用模型，仓库和小程序包内均无模型密钥或 AppSecret；
...
| 4 | AI 情境解读 | 结构化解读、正逆位、爱情/财运/事业/自我成长、反思与行动 | P0 |
| 5 | 本地降级解读 | AI 不可用时组合牌义与牌位模板 | P0 |
| 6 | 本地历史 | 每日牌与完成的主题解读自动保留最近 30 条，支持回看和删除 | P0 |
| 7 | 三牌解读 | 现状—关键影响—行动建议三牌牌阵 | P0 · v0.2 |
| 8 | 回访反馈 | 3—7 天后的结果回顾 | P1 |
| 9 | 图鉴 | 78 张按大阿尔卡那与四花色分组，实际翻牌解锁、总览和详情 | P0 · v1.1 |
| 10 | 分享图 | 隐私安全的牌面和启示图片 | P1 |

### 5.2 方案概述

- **产品方案**：以“每日一牌”和“单牌/三牌”两条主路径满足高频轻体验与中频深体验；单牌/三牌内部默认生活指引，可按需切换为具体问题。
- **当前技术方案**：微信小程序本地领域服务负责抽牌、安全策略、本地历史和降级；单牌/三牌解读通过 CloudBase AI Provider 直接调用 `hy3`，输出仍经相同契约校验。
- **验证方案**：功能测试、AI 离线评估集和小规模体验测试共同验证。

### 5.3 MVP 范围

**包含：**78 张完整牌组正式卡面、正逆位、单牌、三牌、每日一牌、无问题的生活三向指引、CloudBase AI 问题聚焦解读、本地降级、可选本地历史、分组图鉴、安全分类，以及支持定位故障的最小日志。

**暂不包含：**日记、云端历史、显式账号系统、三牌之外的更多牌阵、签到/回访/连续使用循环、完整埋点体系、自动化 E2E、真人服务、社区、支付、会员、广告、星盘/八字、复杂提醒、卡牌养成、运营后台。
...
| 小程序领域服务 | 应用服务 | 抽牌、安全、AI 编排与校验 | 个人开发者 |
| 微信本地存储 | 数据层 | 当前设备的历史和图鉴 | 个人开发者 |
| CloudBase AI / `hy3` | 外部依赖 | 生成情境化解读 | 腾讯云/个人开发者 |
| 小程序静态资源 | 资源层 | 卡牌、卡背与品牌素材 | 个人开发者 |

### 6.2 影响范围

- **用户影响**：仅测试体验者；不面向未成年人主动推广；
- **流程影响**：新建产品，无旧流程迁移；
- **数据影响**：每日牌与已完成的单牌/三牌解读自动进入当前设备最近 30 条历史，均视为私密内容；用户可单条删除；
- **外部影响**：单牌/三牌的牌面事实和解读方式会发送至项目 CloudBase 环境的 AI 服务；仅具体问题模式额外发送用户问题，不读取账号资料或历史记录；
- **部署影响**：正式版依赖已开通 AI 套餐的 CloudBase 环境、基础库 3.15.1 及以上和微信审核配置。
...
| A3 | 78 张完整牌组能在不增加留存循环的前提下提升单次使用惊喜 | 通过重复感反馈和组合可读性观察是否成立 |
...
| 编号 | 约束 | 对设计的影响 |
| --- | --- | --- |
| C1 | 单人、业余时间开发（64—80 小时首版预算，80 小时硬上限） | 使用本地 Repository 与单一 CloudBase AI Provider；超期触发范围熔断 |
...
| 术语 | 定义 |
| --- | --- |
| 大阿尔卡那 | 塔罗牌中的 22 张主要牌，与 56 张小阿尔卡那共同构成本项目 78 张牌库 |
| 小阿尔卡那 | 权杖、圣杯、宝剑、星币四花色各 14 张，共 56 张 |
...
| 实体 | 规则 |
| --- | --- |
| Reading | 每日牌与已完成主题牌自动保存；历史按时间只保留最近 30 条；删除后立即从本地存储移除 |
...
flowchart TD
    ...
    RESULT --> HISTORYSAVE[自动更新最近 30 条本地历史]
    FALLBACK --> HISTORYSAVE
    DAILYRESULT --> HISTORYSAVE
    HISTORYSAVE --> END([结束])
    BLOCK --> END
...
| Completed | 只读 | 只读 | 可见 | 自动保留 | 不可用 | 可用 |
| FallbackCompleted | 只读 | 只读 | 可见并标注降级 | 自动保留 | 可重新请求 AI 一次 | 可用 |
...
| 历史 | 最近 30 条列表、详情和单条删除 | ✓ | P0 |
| 图鉴 | 78 张按五组展示，按翻牌解锁、总览与详情 | ✓ | P0 · v1.1 |
...
**页面要素：**今日卡入口、单牌/三牌入口、最近三条记录、隐私/AI 标识入口。待回访入口为 P1。

| 操作 | 触发条件 | 反馈 | 规则 |
| --- | --- | --- | --- |
| 点击每日一牌 | 今日未抽 | 进入短仪式 | 以本地 Asia/Shanghai 业务日期判定每日唯一 |
| 点击每日一牌 | 今日已抽 | 打开今日详情 | 不允许通过刷新重复抽取 |
| 点击单牌/三牌入口 | 无 | 进入解读设置页，默认生活指引 | 不要求先形成问题；可切换为具体问题 |
...
| 字段 | 类型 | 必填 | 规则 |
| --- | --- | --- | --- |
| 解读方式 | 枚举 | 是 | 默认 `life` 生活指引；可切换为 `question` 具体问题 |
| 问题焦点 | 自动识别 | 条件必填 | 仅具体问题模式启用；不要求用户预选固定类别，模型根据问题识别重点 |
| 问题 | 多行文本 | 条件必填 | 仅具体问题模式展示；5—200 个字符，首尾空格清理，拒绝纯符号和重复垃圾内容 |
...
| 动画 | 支持减少动态效果；动画完成前结果已在 Reading 中固化，退出重进仍保持一致 |

#### 10.2.4 AI 情境解读

> 💡 确定性-容错率分析：这是探索性、相对高容错但涉及情绪安全的任务，采用“CUI 能力嵌入固定 GUI 结果页”，不使用全对话替代界面。

真实 AI 用于单牌/三牌：生活指引模式不伪造用户问题，只提交固定方向、牌阵、卡牌事实与受控牌义；具体问题模式再额外提交当前问题。每日一牌继续使用静态牌义、财运/事业/爱情三条受控方向提示和预置反思问题。小程序通过 CloudBase 内置 Provider `cloudbase` 调用 `hy3`，不提交账号资料或历史记录。具体问题应视为会发送给外部 AI 处理的数据，界面提示用户不要输入姓名、电话、住址等可识别信息；CloudBase 控制台可能保留调用记录。

AI 必须返回以下结构。具体问题模式逐牌只返回一条问题聚焦解读；生活指引模式逐牌固定返回财运、事业、爱情三条 `directionInsights`，结果页通过页签一次只展示一条，不恢复多维长文墙：

| 字段 | 类型 | 规则 |
| --- | --- | --- |
| summary | string | 不超过 260 字，直接给出新的核心观察；不得复述、引用或套话改写用户问题 |
| cards | array | 数量与抽牌数一致；逐张包含牌名、牌位、依据、情境解释和模式对应洞察；生活指引的方向键与顺序必须为 `wealth`/财运、`career`/事业、`love`/爱情 |
| synthesis | string | 不超过 320 字；把三张牌连成现状—关键影响—行动建议，单牌则收束观察角度 |
| reflection_question | string | 只能有一个开放式反思问题 |
| micro_action | string | 一个 24—72 小时内可执行、低风险、非操纵他人的行动 |
| disclaimer | string | 必须精确等于程序给定的 AI 免责声明，否则降级 |
...
人机边界：`hy3` 提供解释与反思建议，用户可以不认同并删除本地记录；产品不展示虚构置信度，不保存或展示模型思维链。

#### 10.2.5 解读结果与降级

结果页按“总览 → 可选的原始困扰 → 第一眼线索 → 牌阵与当前牌解读一体区 → 可展开牌义依据 → 综合提示 → 反思问题与微行动 → 可选分享”渐进展示。生活指引的三个方向使用同一区域内的页签切换，一次只出现一段；三牌默认只展开一张，真实卡面直接作为当前解读的就近切换入口。免责声明降为辅助信息，不抢占首屏，页面不再展示 AI 来源标识。结果页不设置额外“完成”按钮，退出统一使用系统左上角返回或侧滑返回。
...
- 每日牌自动保存，用于当天复用；
- 完成的单牌/三牌解读（包括安全降级结果）自动保存；
- 每次写入后按完成时间保留最近 30 条，多出的旧记录立即从本地移除；
...
图鉴按大阿尔卡那、权杖、圣杯、宝剑、星币五组展示 78 张牌。实际翻开后才解锁图像和详情，并记录翻开次数、已见正逆位；未解锁卡使用统一卡背遮挡。详情展示图像、关键词、正位含义、逆位含义和一个自我反思问题。图鉴独立于历史，不提供系统课程、考试、签到或付费养成。

不设置独立设置页。AI 与敏感信息边界在解读设置页就近提示，其中个人信息提示只在具体问题模式显示；娱乐性声明在结果页展示，历史页直接说明最近 30 条与仅存当前设备的边界。

### 10.3 异常情况处理

| 异常类型 | 场景 | 处理方案 |
| --- | --- | --- |
| 弱网/断网 | 抽牌前断网 | 本地抽牌仍可完成；AI 调用失败后展示基础牌义 |
| 弱网/断网 | 抽牌已完成、解读中断 | 复用同一 Reading，不重新抽牌，并展示降级内容 |
| 重复点击 | 连续点击生成 | 页面按钮进入 loading；同一 Reading 已有结果时直接复用 |
| AI 超时 | 超过客户端 25 秒硬超时 | 不展示迟到响应，直接使用降级解读 |
| AI 非法结构 | JSON 缺字段或牌面不一致 | 不展示原始输出；记录原因码并降级 |
| AI 不安全输出 | 命中输出过滤 | 使用安全模板或阻断，不保存原始输出 |
| CloudBase 异常 | AI 调用失败 | 保留抽牌事实并立即生成本地降级结果 |
| 本地存储不可用 | 写入或读取失败 | 当前结果仍可查看；明确提示未保存，不伪装成功 |
| 数据冲突 | 同设备重复写入 | 以 Reading ID 更新本地记录；v1.0 不做跨设备同步 |
| 数据删除 | 用户删除历史或图鉴 | 从微信本地存储立即移除；CloudBase AI 调用记录按控制台策略另行管理 |
...
| history_update | 完成结果写入最近历史 | reading_type, source, retention_count | 自动历史写入与淘汰 |
...
- v1.0 不提供远程历史读写接口；页面只能访问当前设备 Repository 中的 Reading；
...
| 开发版 | 开发者 | 验证 CloudBase AI 和本地数据闭环 | 自动化测试通过，真实模型探测成功 | 使用本地降级 |
...
| --- | --- | --- | --- | --- | --- |
| TBD-1 | 正式产品名称与视觉关键词 | 5/10 | 项目作者 | 2026-07-28 | 已调整：微信审核对外名为“心镜拾光”；保留黑灰版画、纤维纸、旧金箔 |
| TBD-2 | 确认 CloudBase 套餐、资源地域与预算 | 6/10 | 项目作者 | 工程搭建前 | 已确认：环境 `<your-cloudbase-env-id>`，AI 套餐已开通 |
| TBD-3 | 选择主模型与预算上限 | 7/10 | 项目作者 | AI Spike 第 0 项 | 主模型已定 `hy3`；备模型与用量告警待发布后补充 |
| TBD-4 | 78 张牌使用何种原创/授权视觉资产 | 7/10 | 项目作者 | 已关闭 | 已完成 A“仪式典藏”22+56 张逐牌生成、联排与技术验收 |
| TBD-5 | 本地历史是否迁移到云端 | 10/12 | 项目作者 | 云端阶段前 | 已决：默认不自动迁移 |
| TBD-6 | 是否以真实小程序审核上线为里程碑 | 6/13 | 项目作者 | 内测前 | 已确认：以正式版提交微信审核为当前里程碑 |
...
1. 已验证 CloudBase 环境中的 `hy3` 可达并能返回中文内容；发布前继续验证完整结构化单牌和三牌流程；
2. 确认牌面素材授权或原创计划；
3. 契约和安全自动化测试覆盖单牌、三牌与降级；提交审核前完成至少 40 条单牌/三牌真实 AI 发布评估集；
