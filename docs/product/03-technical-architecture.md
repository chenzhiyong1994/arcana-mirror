...
apps/miniprogram/
  ...
  components/tarot-card-face/      # 大牌透传、小牌共享边框与身份叠加
  assets/card-thumbs/              # 主包 78 张 192×288 缩略图
  assets/cards/                    # 主包卡背 + 384×576 共享透明正面框
  packages/deck/
    pages/                         # 仪式、结果、历史、图鉴
    assets/cards/                  # 78 张 384×576 展示图 + 卡背
assets/tarot-card-style/
  shared/                          # 1024×1536 正面框与卡背原件
...
| 数据 | 必要性 | 保存 | 日志 | 删除 |
| --- | --- | --- | --- | --- |
| 问题 | 生成解读 | 完成后随 Reading 自动进入当前设备最近 30 条历史 | 应用不主动记录；CloudBase 可能保留调用记录 | 用户删除本地 Reading；云端记录按控制台策略 |
| 抽牌事实 | 保证一致与回看 | 随本地 Reading 保存 | 应用不主动上传日志 | 随本地 Reading 删除 |
| AI 解读 | 展示和回看 | 完成后随本地 Reading 自动保存，并随最近 30 条策略淘汰 | 应用不主动记录完整响应；CloudBase 可能保留调用记录 | 随本地 Reading 删除；云端记录按控制台策略 |
